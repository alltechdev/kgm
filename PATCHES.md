...
32. **Onboarding/empty-state nudges silenced.**
    * **"Start your chat / Here's a few ideas to get you started" + "Add people" + "Share link"** — `ChatFragment.updateEmptyState()` body replaced with an unconditional sequence that calls `setVisibility(GONE)` on `mEmptyStateTopicDMView`, `mEmptyStateGroupView`, and `mEmptyStateCopilotView`, then returns. The original body is left as dead code after the `return-void`.
    * **"Introducing Topics / Organize your group's chat"** — `ChatDetailsFragment.toggleIntroPill()` forces `v0 = 0` right after `isTopicsEnabled()`'s `move-result`, so the very first `if-eqz` always jumps to the hide branch.
    * **Calendar row in group details** — `ChatDetailsListAdapter.onBindViewHolder` now checks at method entry whether the current item is `ChatDetailsItem.Calendar`; if so, `holder.itemView` is set to GONE, its `LayoutParams.height` is zeroed, and the method returns — the row collapses to zero height.
33. **Microsoft Teams calling stripped app-wide.**
    * `IncomingCallActivity.onCreate` (the GroupMe inbound-call popup) and `com.azure.android.communication.ui.calling.presentation.CallCompositeActivity.onCreate` (the Azure Communication UI screen — the actual "Call with Microsoft Teams" UI) both call `Activity.finish()` immediately after their `super.onCreate`. Any intent that targets them is a no-op flash.
